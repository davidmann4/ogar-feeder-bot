...
FeederBot.prototype = {
    ...
    playerInRange: function(my_ball, playerX, playerY, playerSize, range) {
        var bot = this;
        bot_distance = bot.getDistanceBetweenBallAndPosition(my_ball, playerX, playerY) - bot.getMassPixelRadius(bot.client.balls[player_cell_id].size)
    ...
    recalculateTarget: function() {
        ...
        var my_ball = bot.client.balls[bot.client.my_balls[0]];
        var player_ball = bot.client.balls[player_cell_id];
        if (!my_ball) return;
        if (!player_ball)
        {
            bot.client.moveTo(Math.floor((Math.random() * -2500) + 2500), Math.floor((Math.random() * -2500) + 2500));
            return;
        }

        if (bot.isOnFeedMission) {

            if (config.enableSaveMoveTo) {
                bot.safeMoveTo(player_ball.x, player_ball.y);
            } else {
                bot.client.moveTo(player_ball.x, player_ball.y);
            }

            if (bot.playerInRange(my_ball, player_ball.x, player_ball.y, player_ball.size, 400)) {
                if (bot.canSplitFeedPlayer(my_ball.mass, player_ball.size)) {
                    bot.client.split();
                }
            }
        ...
        if (my_ball.mass > config.minimumMassBeforeFeed) {
            bot.isOnFeedMission = true;
            return;
        }

        if (bot.playerInRange(my_ball, player_ball.x, player_ball.y, player_ball.size, 1000)) {

            if (!got_tranporter ||
                bot.getDistanceBetweenBalls(candidate_ball, my_ball) >
                bot.getDistanceBetweenBallAndPosition(my_ball, player_ball.x, player_ball.y)
            ) {
                bot.isOnFeedMission = true;
                return;
            }
